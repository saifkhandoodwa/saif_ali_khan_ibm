...
Emergency Vehicle Preemption, and Safe All-Red Clearance Interval.
"""
...
class TrafficController:
    """
    Intelligent Agent controlling traffic signals with anti-collision clearance buffers.
    """
    ...
    def __init__(
        ...
        all_red_time: float = 2.0,  # Critical All-Red buffer to clear intersection
    ...
    def calculate_phase_demand(self, telemetry: Dict[Direction, LaneTelemetry], phase: TrafficPhase) -> float:
        ...
        Formula: Demand = sum(load * 1.5 + avg_wait * 1.8 + waiting_count * 2.5)
        """
    ...
    def step(self, dt: float, telemetry: Dict[Direction, LaneTelemetry]):
        """
        Simulation tick logic. Handles timers, safe transitions, and emergency preemption.
        """
        ...
        if self.light_state == LightState.GREEN:
            if self.mode == ControlMode.AI_ADAPTIVE:
                ...
                # Early switch if active corridor is empty
        ...
        elif self.light_state == LightState.ALL_RED:
            if self.phase_time_elapsed >= self.all_red_time:
                self._switch_phase(telemetry)

    def _handle_emergency(self, priority_dir: Direction, dt: float):
        """Preempts signals safely to clear passage for emergency vehicle."""
        ...
        if self.active_phase != target_phase:
            if self.light_state == LightState.GREEN:
                ...
                self._transition_to_all_red()
            elif self.light_state == LightState.ALL_RED and self.phase_time_elapsed >= self.all_red_time:
    ...
    def _transition_to_all_red(self):
        """All-Red clearance: halts all lanes to ensure intersection box is 100% empty."""
